...
void l22::xml_writer::do_size_of_node(l22::size_of_node * const node, int lvl) {
  openTag(node, lvl);
...
void l22::xml_writer::do_write_node(l22::write_node * const node, int lvl) {
  ...
  if (node->nLine()) {
      os() << std::string(lvl, ' ') << "<" << node->label()<<"_nLine" << ">" 
          << newLine(node->nLine()) << "</" << node->label()<<"_nLine" << ">" << std::endl; // TO CHECK
  }
  if (node->argument()) node->argument()->accept(this, lvl + 2);
  closeTag(node, lvl);
}
...
void l22::xml_writer::do_null_pointer_node(l22::null_pointer_node * const node, int lvl) {
  openTag(node, lvl);
  closeTag(node, lvl);
}
...
void l22::xml_writer::do_variable_declaration_node(l22::variable_declaration_node * const node, int lvl) {
  openTag(node, lvl);
    os() << std::string(lvl+2, ' ') << "<" << node->variableId() << " = " << node->initializer() << "; type = " << 
...
void l22::xml_writer::do_function_call_node(l22::function_call_node * const node, int lvl) {
  ...
  openTag(node, lvl);
    os() << std::string(lvl+2, ' ') << "<" << node->label() << " name='" << node->function()->label() << "'>" << std::endl;
...
void l22::xml_writer::do_function_declaration_node(l22::function_declaration_node * const node, int lvl) {
  openTag(node, lvl);

  os() << std::string(lvl+2, ' ') << "<" << node->label() << " name='" << node->identifier() << "' qualifier='"
